 ...
 *
 * Self-animates with a subtle imageReveal (scale + fade in) the first time
 * it enters the viewport, and optionally offers a soft hover zoom for
 * gallery-style tiles — the two never fight over the `transform` property
 * because the hover scale lives on an inner wrapper, independent of the
 * outer element's one-time entrance transform.
 */
export function PhotoPlaceholder({
  ...
  compact?: boolean;
  /** Soft zoom on hover (desktop pointer only) — use for gallery-style tiles, not full hero backgrounds. */
  hoverScale?: boolean;
  /** Stagger offset (ms) for the entrance reveal — use in grids/masonry so tiles settle in sequence. */
  delay?: number;
  className?: string;
}) {
  ...
  return (
    ...
    <div
      ref={ref}
      className={`reveal reveal--image-reveal overflow-hidden ${inView ? "is-visible" : ""} ${isNavy ? "bg-navy" : "bg-off-white"} ${className}`}
    ...
    >
      <div
        className={`relative h-full w-full ${
          hoverScale ? "transition-transform duration-500 ease-out [@media(hover:hover)]:hover:scale-[1.025]" : ""
        }`}
      >
        {/* Base tone */}
        <div
